...
int main() {
  ...
  propeller_params.m_thruster_perf_data_json_string = ktkq_json_string;
  ...
  rudder_params.m_perf_data_json_string = clcdcn_json_string;

  /**
   * Brix Propeller rudder construction
   */

  std::cout<<"<<<<<<<<<<<< BRIX prop-rudder >>>>>>>>>>>>>>"<<std::endl;

  auto propeller_rudder = build_Brix_pr(E_FPP1Q, propeller_params,
                                        E_SIMPLE_RUDDER, rudder_params);
  ...
  std::cout << "Thrust: " << propeller_rudder->GetPropellerThrust() * 1e-3 << " kN" << std::endl;
  std::cout << "Torque: " << propeller_rudder->GetPropellerTorque() * 1e-3 << " kN.m" << std::endl;
  std::cout << "Efficiency: " << propeller_rudder->GetPropellerEfficiency() << std::endl;
  std::cout << "Power: " << propeller_rudder->GetPropellerPower() * 1e-3 << " kW" << std::endl;

  std::cout << "Propeller-Rudder Fx: " << propeller_rudder->GetPropellerRudderFx() * 1e-3 << "kN" << std::endl;
  std::cout << "Propeller-Rudder Fy: " << propeller_rudder->GetPropellerRudderFy() * 1e-3 << "kN" << std::endl;
  std::cout << "Propeller-Rudder Mz: " << propeller_rudder->GetPropellerRudderMz() * 1e-3 << "kNm" << std::endl;


  /**
   * MMG Propeller rudder construction
   */
  std::cout<<"<<<<<<<<<<<< MMG prop-rudder >>>>>>>>>>>>>>"<<std::endl;

  auto MMG_propeller_rudder = build_MMG_pr(E_FPP1Q, propeller_params,
                                        E_SIMPLE_RUDDER, rudder_params);


  MMG_propeller_rudder->Initialize();


  // Calculation


  MMG_propeller_rudder->Compute(water_density,
                            u_NWU_propeller_ms,
                            v_NWU_propeller_ms,
                            u_NWU_ship_ms,
                            v_NWU_ship_ms,
  ...
  std::cout << "Thrust: " << MMG_propeller_rudder->GetPropellerThrust() * 1e-3 << " kN" << std::endl;
  std::cout << "Torque: " << MMG_propeller_rudder->GetPropellerTorque() * 1e-3 << " kN.m" << std::endl;
  std::cout << "Efficiency: " << MMG_propeller_rudder->GetPropellerEfficiency() << std::endl;
  std::cout << "Power: " << MMG_propeller_rudder->GetPropellerPower() * 1e-3 << " kW" << std::endl;

  std::cout << "Propeller-Rudder Fx: " << MMG_propeller_rudder->GetPropellerRudderFx() * 1e-3 << "kN" << std::endl;
  std::cout << "Propeller-Rudder Fy: " << MMG_propeller_rudder->GetPropellerRudderFy() * 1e-3 << "kN" << std::endl;
  std::cout << "Propeller-Rudder Mz: " << MMG_propeller_rudder->GetPropellerRudderMz() * 1e-3 << "kNm" << std::endl;
